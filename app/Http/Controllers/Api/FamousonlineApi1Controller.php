...
class FamousonlineApi1Controller extends Controller
{
        ...
        if (empty($authKeyEncryted)) {
            ...
            if (!$authokey) {
                ...
                $data = [
                    ...
                    "app_download_link" => "https://www.bestsattamatka.net/download-app/com.bestsattamatka.famousonline",
            ...
            if (!$authokey) {
                ...
                if (!isset($rawdata['reg_no']) || !isset($rawdata['token'])) {
                    ...
                    $user = DB::table('users_master')
                        ...
                        ->where('source', $this->appname)
                        ->first();
                    $userBlocked = DB::table('users_master')
                        ->where('reg_no', $reg_no)
                        ->where('token', $token)
                    ...
                    }else if (!empty($userBlocked)) {
                        echo json_encode([
                            "code" => "2",
                            "msg" => 'Note - We would like to inform that currently your account is Block hold due to policy violation or suspicious activity from your account.',
                        ]);
                    }  else {
                    ...
                    }else if ($this->today_date != $lottery->lottery_date) 
                    {
                        ...
                        switch ($game_type) {
                            ...
                            case 'Close':
                                ...
                                if ($lottery->close_time =="12:05AM") {
                                    $bet_close_time = strtotime("11:59PM");
                                }else
                                {
                                    $bet_close_time = strtotime($lottery->close_time);
                                }
